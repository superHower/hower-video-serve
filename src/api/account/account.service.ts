...
export class AccountService {
   ...
   * @return {*}
   */
  async createAccountApi(req: AccountDto): Promise<string> {
    console.log('进来了------------------》》', req);
    // 1.判断当前商户下是否已经存在该账号
  ...
  get queryAccountBuilder(): SelectQueryBuilder<AccountEntity> {
    return this.accountRepository
      ...
      .addSelect('account.nickname', 'nickname')
      .addSelect('account.avatar', 'avatar') 
      .addSelect('account.info', 'info')
      .addSelect('account.gender', 'gender')
      .addSelect('account.age', 'age')
